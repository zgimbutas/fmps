...
nspheres = 2; r0=5; sep=1.01;
[center,radius]=spheres_mi_geo(nspheres,r0,sep);
...
sol = gmres_simple(@(x) em3d_multa_fmps(A,x), rhs, 1e-12, 40);
...
if( 2 == 2 ),
%
%  Correct polarizability factors
%
[pvec,mvec]=empolms(wavelength,zk,nspheres,aompole,bompole,nterms,center,radius);

pdipole(1:3,ifreq) = pvec;
mdipole(1:3,ifreq) = mvec;
...
figure(4);
x = scan_range;
plot( gold_jc(x,2), -real(qabs_total(x)))
title('absorption cross section (SI)')
grid on
end
...
figure(1);
x = scan_range;
plot( gold_jc(x,2), real(pdipole(1,x)), gold_jc(x,2), real(pdipole(2,x)), gold_jc(x,2), real(pdipole(3,x)))
title('real part electric polarization vector')
grid on

figure(2);
x = scan_range;
plot( gold_jc(x,2), imag(pdipole(1,x)), gold_jc(x,2), imag(pdipole(2,x)), gold_jc(x,2), imag(pdipole(3,x)))
title('imag part electric polarization vector')
grid on
end

figure(1);
x = scan_range;
plot( gold_jc(x,2), (real(pdipole(1,x))))
title('real part electric polarization vector')
grid on
hold on
...
figure(2);
x = scan_range;
plot( gold_jc(x,2), (imag(pdipole(1,x))))
title('imag part electric polarization vector')
grid on
hold on
...
figure(3);
x = scan_range;
semilogy( gold_jc(x,2), abs(real(pdipole(1,x))))
title('real part electric polarization vector')
grid on
hold on
...
figure(4);
x = scan_range;
semilogy( gold_jc(x,2), abs(imag(pdipole(1,x))))
title('imag part electric polarization vector')
grid on
hold on
