...
%
%  Plot Re(E_z^{incoming}) on all spheres
%
figure(101);
for isphere = 1:nspheres
    
f = squeeze((eivecs(3,:,:,isphere)));
fp = [f; f(1,:)];
...
ylabel('y')
zlabel('z')
title('Re(E_z^{incoming})')
figure(101)


%
%  Plot Re(E_z^{total}) on all spheres
%
figure(102);
for isphere = 1:nspheres
    
f = squeeze((etvecs(3,:,:,isphere)));
fp = [f; f(1,:)];
...
%
%  Plot Re(E_z^{scat}) on all spheres
%
figure(103);
for isphere = 1:nspheres
    
f = squeeze((esvecs(3,:,:,isphere)));
fp = [f; f(1,:)];
