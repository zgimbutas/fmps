function [ra,rb]=rcoefs_die_arb12(nterms,omega,r,eps1,cmu1,eps2,cmu2)
%RCOEFS_DIE_ARB12 Construct reflection coefficients for a dielectric sphere.
%
%  [ra,rb]=rcoefs_die_arb12(nterms,omega,r,eps1,cmu1,eps2,eps2);
%
...
%
%  Reflection coefficients: TM_12, TE_12
%
ra = -(sqrt(eps1).*sqrt(cmu2).*emthvals1(k).*hvals2(k)-   ...
       sqrt(eps2).*sqrt(cmu1).*emthvals2(k).*hvals1(k))./ ...
      (sqrt(eps1).*sqrt(cmu2).*emtjvals1(k).*hvals2(k)-   ...
       sqrt(eps2).*sqrt(cmu1).*emthvals2(k).*jvals1(k));

rb = -(sqrt(eps2).*sqrt(cmu1).*emthvals1(k).*hvals2(k)-   ...
       sqrt(eps1).*sqrt(cmu2).*emthvals2(k).*hvals1(k))./ ...
