...
c
        ...
        y3 = verts(2,3)
c
        call linedist(x1,y1,x2,y2,dd1)
        call linedist(x2,y2,x3,y3,dd2)
        call linedist(x1,y1,x3,y3,dd3)
c
        rcut = min(dd1,dd2)
        rcut = min(dd3,rcut)
        rcut = rcut/2
...
c
        call radquad0(norder,n,rcut,x1,y1,x2,y2,
     -   nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1),
     -   ts,twhts)
        nquad = nquad+nquad0
c
        call radquad0(norder,n,rcut,x1,y1,x3,y3,
     -    nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1),
     -   ts,twhts)
        nquad = nquad+nquad0
c
        call radquad0(norder,n,rcut,x2,y2,x3,y3,
     -    nquad0,xs(nquad+1), ys(nquad+1),whts(nquad+1),
     -   ts,twhts)
        nquad = nquad+nquad0
c
        call raddiag_disk0(n,norder,rcut,
     -    nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
        nquad = nquad+nquad0
c
        end
        ...
        subroutine radquad0(norder,n,rcut,x1,y1,x2,y2,
...
c
c
 0050 format ("pvradquads",I2.2,".txt")
 0100 format (I3.3)
 0200 format (D44.36)
c
        ier   = 0
...
c        
        ...
        call prinf("in radial_init, nr = *",nr,1)
        call prinf("in radial_init, nt = *",nt,1)
ccc        call prin2("in radial_init, rs = *",rs,2*nr)
ccc        call prin2("in radial_init, ts = *",ts,2*nt)
c
...
c
        ...
        n     = 3*(norder+1)+2
c
        call raddiag_disk0(n,nlege,rcut,
     -    nquad0,xs(nquad+1),ys(nquad+1),whts(nquad+1))
c
        nquad = nquad+nquad0
        end
        ...
        subroutine raddiag_disk0(n,nlege,rcut,
     -    nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
...
c
c
        subroutine raddiag0(ier,rad,x1,y1,x2,y2,nquad,xs,ys,whts,rcut,
     -   norder)
        implicit double precision (a-h,o-z)
        dimension rad(1),xs(1),ys(1),whts(1),verts(2,3)
c
