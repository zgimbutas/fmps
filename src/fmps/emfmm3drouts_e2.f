...
c        of the routines for Maxwell FMM in R^3
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
c
c
        subroutine emfmm3dparttree(ier,iprec,zk,
...
c       
c       Maxwell FMM in R^3: build the oct-tree
c
c     INPUT PARAMETERS:
c
...
c
            call em3dzero(rmlexp(iaddr(1,ibox)),nterms(level0))
            if (ifprint .ge. 2) then
...
c               call h3dmpmpquadu_add(zk,scale(level1),center1,
c     1            rmlexp(iaddr(1,jbox)),nterms(level1),scale(level0),
c     1            center0,rmlexp(iaddr(1,ibox)),
c     $            nterms(level0),nterms(level0),
cccc     1            radius,xnodes,wts,nquad,ier)
c     1            radius,xnodes2,wts2,nquad2,ier)

        call em3mpmp3_add(zk,
     $     center1,
     $     rmlexp(iaddr(1,jbox)),rmlexp(iaddr(3,jbox)),nterms(level1),
     $     center0,
     $     rmlexp(iaddr(1,ibox)),rmlexp(iaddr(3,ibox)),nterms(level0),
...
c
ccc        call em3dformmp(ier,scale(0),source,charge,n,
ccc     1  	center,nterms(0),mptemp)
c
ccc        call prinm(mptemp,nterms(0))
c
ccc        call em3dmperr(rmlexp(iaddr(1,1)),mptemp,nterms(0),d)
...
c
     ...
     $     rmlexp(iaddr(1,jbox)),rmlexp(iaddr(3,jbox)),nterms(level1),
     $     center0,
     $     rmlexp(iaddr(2,ibox)),rmlexp(iaddr(4,ibox)),nterms(level0),
     $     radius,rnodes,rwts,nphi,ntheta)
...
c
	          ...
	          if (jbox.eq.0) goto 5100
                  ...
                  level1=box1(1)
c	          call h3dloclocquadu_add(zk,scale(level0),center0,
c     1    	      rmlexp(iaddr(2,ibox)),nterms(level0),
c     1                scale(level1),center1,rmlexp(iaddr(2,jbox)),
c     $                nterms(level1),nterms(level1),
cccc     1    	      radius,xnodes,wts,nquad,ier)
c     1    	      radius,xnodes2,wts2,nquad2,ier)

        call em3tata3_add(zk,
     $     center0,
     $     rmlexp(iaddr(2,ibox)),rmlexp(iaddr(4,ibox)),nterms(level0),
     $     center1,
     $     rmlexp(iaddr(2,jbox)),rmlexp(iaddr(4,jbox)),nterms(level1),
     $     radius,rnodes,rwts,nphi,ntheta)
...
c
c
        subroutine em3dpsort(n,isource,psort,pot)
        implicit real *8 (a-h,o-z)
        dimension isource(1)
...
c
c
        subroutine em3dfsort(n,isource,fldsort,fld)
        implicit real *8 (a-h,o-z)
        dimension isource(1)
...
c
c
        subroutine em3dpsortsub(n,isource,psort,pot)
        implicit real *8 (a-h,o-z)
        dimension isource(1)
        complex *16 pot(1),psort(1)
c
...
c
c
        subroutine em3dfsortsub(n,isource,fldsort,fld)
        implicit real *8 (a-h,o-z)
        dimension isource(1)
        complex *16 fld(3,1),fldsort(3,1)
c        
...
c
c
        subroutine em3dreorder(nsource,source,
...
c
c
        subroutine em3dreordertarg(ntarget,target,itarget,targetsort)
        implicit real *8 (a-h,o-z)
...
c
c
        subroutine em3dreordertria(nsource,isource,
...
c
c
        subroutine em3dzero(mpole,nterms)
        implicit real *8 (a-h,o-z)
c
...
c
c
        subroutine em3dmpalloc(wlists,iaddr,nboxes,lmptot,nterms)
        implicit real *8 (a-h,o-z)
        integer box(20)
...
c
        ...
        level=box(1)
c
c       ... first, allocate memory for the a-multipole expansion
c       
        iaddr(1,ibox)=iptr
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
c
c       ... then, allocate memory for the a-local expansion
c       
        iaddr(2,ibox)=iptr
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
c       
c       ... then, allocate memory for the b-multipole expansion
c       
        iaddr(3,ibox)=iptr
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
c
c       ... then, allocate memory for the b-local expansion
c       
        iaddr(4,ibox)=iptr
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
c       
